...
<div class="BackBox" style="text-align: left;background-color: rgba(0,0,0,.7);">
    <form method="POST" action="../includes/addDataFunction.inc.php">
        ...
        <input class="inputBox" type="text" style="text-transform: uppercase" maxlength="2"  name="courseGrade" placeholder="(Optional) Example: 'A+' 'F'"><br/><br/>
        <label for="studentDropDown">Student ID: </label>
        <select class="dropDown" name="studentDropDown">
            <option>None</option>
            <?php
            require '../includes/dbCentralConnect.inc.php';
            $findAllStudent='SELECT ID,name FROM student;';
            $stmt=mysqli_stmt_init($conn);
            if(!mysqli_stmt_prepare($stmt,$findAllStudent)){
                echo '<div class="errorBG">';
                echo '<span class="errorType1">Information fetch failed!</span>';
                echo '<span class="errorType1"> Contact the Webmaster</span>';
                echo '</div>';
        ...
        <button class="buttons" style="margin-left: 50px" name="assignCourseToStudent" type="submit">Assign to Student</button><br/>
        <button class="buttons" style="background-color: rgba(127,0,0,1);margin-left: 50px" type="reset">Reset</button>
    </form>
</div>
<div class="errorBG"><span class="errorType1"><strong>Warning: </strong>Confirm the details before clicking 'Assign to Student'</span></div>
</body>
</html>